...
public class RingBufferImpl implements RingBuffer {
     ...
     */
    public RingBufferImpl(int bufferSize) {
     ...
     */
    public RingBufferImpl(int bufferSize, int paddingFactor) {
        // check buffer size is positive & a power of 2; padding factor in (0, 100)
        UGAssertUtils.isTrue(bufferSize > 0L, "RingBuffer size must be positive");
        UGAssertUtils.isTrue(Integer.bitCount(bufferSize) == 1, "RingBuffer size must be a power of 2");
        UGAssertUtils.isTrue(paddingFactor > 0 && paddingFactor < 100, "RingBuffer size must be positive");
    ...
    public long take() {
        ...
        UGAssertUtils.isTrue(nextCursor >= currentCursor, "Curosr can't move back");
        ...
        UGAssertUtils.isTrue(flags[nextCursorIndex].get() == CAN_TAKE_FLAG, "Curosr not in can take status");
